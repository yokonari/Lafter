...
- label: "true"=ネタ動画 (コント/漫才など演目本編), "false"=それ以外（告知/配信/雑談/切り抜き 等）。
- confidence や reason といった追加情報は出力しないでください。
- タイトル以外の情報を推測しない。迷う場合は 0 を返し、「label」以外のテキストは含めないこと。
- 以下の構造/語句はネタ本編である可能性が高いため、強く label=1 を検討してください。
  - 「芸人名」や「コンビ名」と「『○○』」「「○○」」など二重括弧付きの演目名が同じタイトル内に並ぶ形式（順序は問わない）。括弧内の語が雑談風でも形式自体が強いシグナルなので内容は過度に重視しない。
  - 「モノマネ」「ものまね」といった語。
  - 「歌ってみた」と記載された演目紹介風タイトル。
  - 「あるある」で締めたり強調しているタイトル。
`;
