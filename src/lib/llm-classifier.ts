...
const PROMPT_HEADER = `
あなたは「お笑い動画のタイトル」を分類する専門家です。目的: タイトルが「ネタ動画（漫才・コントなどの本編）」かどうかを判定すること。必ず json 形式 {"label":"true"|"false"} で応答し、余計なキーは一切追加しないでください。
...
- タイトル以外の情報を推測しない。ただし、このドメインは「お笑い芸人の公式チャンネル上の動画」であり、
  タイトルがネタの題名らしく見える場合は積極的に "true" を検討してください。
- 次のようなパターンは、他に強いNG要素（「生配信」「やってみた」など）がない限り、ネタ本編である可能性が高いとみなし、原則 "true" を返します:
  - 「芸人名」や「コンビ名」と「『○○』」「「○○」」「【○○】」などの括弧付き演目名が並ぶ形式（例: 四千頭身「昨日の晩ごはん」）。
    括弧内が短くても構わない。
  - 単語1〜2語だけで構成された非常に短いタイトル（例: 「カフェ」「パンダ」など）で、
    「やってみた」「生配信」「ゲーム」など企画・配信系を示す語が含まれていないもの。
  - 「漫談」「公式」「ネタ」「モノマネ」「ものまね」「歌」「あるある」といった語が演目紹介的に使われている場合。
  - 「〜なやつ」「〜な時」「〜な人」のようにシチュエーションだけで説明されているタイトル。
- 次の語が含まれる場合は企画/バラエティ色が強く、原則 label="false":
  - 「やってみた」「してみた」「聞いてみた」「企画」「トーク」「チャレンジ」「放送事故」「密着」
    「生配信」「生放送」「ゲーム実況」「作業用」「切り抜き」「切り取り」「ダイジェスト」「まとめ」など。
- 「「」「」【】『』」など括弧が含まれるタイトルは必ず LLM 判定対象とみなし、括弧だけで判断せず全体の文意を精査する。
- 上記いずれにも強く当てはまらず、「企画・配信」か「ネタ本編」か全く判断がつかない場合のみ "false" を返してください。
